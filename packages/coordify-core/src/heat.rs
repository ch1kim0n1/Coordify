...
mod tests {
    ...
    #[test]
    fn intent_only_is_monitor_and_recommendation() {
        let cfg = HeatConfig::default();
        let k = Knowledge::default();
        // same intent only (0.15 -> 15) plus same time (temporal 1.0 -> 0.10 -> 10) = 25 -> SAFE boundary.
        // Use different time to isolate intent: last_seen far apart so temporal ~0.
        let a = inputs("agent-1", "BUGFIX", &["a"], &["X"], "p", 0, None);
        let b = inputs("agent-2", "BUGFIX", &["b"], &["Y"], "q", 10_000_000, None);
        let r = compute_heat(&a, &b, &k, &cfg);
        // intent .15 only = 15 -> MONITOR
        assert_eq!(r.heat, 15);
